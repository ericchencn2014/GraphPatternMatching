...
//		printf("userInit, vertexSetSize :%d\n", myDataManager->vertexSetSize());
...
//		printf( " myDataManager->iterateData\n");

		while ((kkk = myDataManager->iterateData(false)) != 0) {
//			printf( " kkk = myDataManager->iterateData(false))\n");
...
//				printf("finish singleVertexStep\n");
...
//		printf( " getSoftDynamicVertexSendSize\n");

		for (int i = 0; i < myDataManager->getSoftDynamicVertexSendSize();
				i++) {

//			printf( " kkk = myDataManager->getSoftDynamicVertexSendSize())\n");
			/*if (i == 0) {
			 <<<<<<< .mine
			 cout << "\t-PE " << sc->getRank()
...
//			printf( " isEmptyRejectedMessageQueue\n");
