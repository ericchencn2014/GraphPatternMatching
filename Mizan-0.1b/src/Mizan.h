...
//		printf("~~~~~~~~%d ,  %s, %s, %s\n",enableVertices, iWantToTerminate?"true":"false",next?"true":"false", stealEnabled?"true":"false");
...
//				printf("else stealEnabled\n");
	...
	void terminateMizan() {
//		cout <<" rank  " <<  myRank << " is terminating.." << endl;
		if (doWriteToDisk) {
...
//		printf("endOfSSProc messageSyncLock\n");
...
//		printf("bodyRecvEndOfSSTerminate\n");
...
//					<< minMem << " -----" << std::endl;
	...
	void ResetMess(){
		sysGroupMessageCounter[StartSS] = 1;
	}
...
//		printf("messageSyncLock recvStartSS\n");
		messageSyncLock.lock();
...
//		printf(" %d, %d\n", sysGroupMessageCounter[StartSS], PECount);
...
//		InitWh(argc, argv)
	...
	void InitWh(int argc, char** argv){


//		SSSPCombiner sprc;
//		groupVoteToHalt = true;
//		registerMessageCombiner(&sprc);
//
//		setVoteToHalt(groupVoteToHalt);
//
//		//User Defined aggregator
//		char * maxAggA = "maxAggregatorA";
//		maxAggregator maxi;
//		registerAggregator(maxAggA, &maxi);
//
//
...
//		}
	...
	void ResetsuperStepCounter(int n){
		superStepCounter = n;
	}
