...
object SubgraphIso extends App{

    def calculate(adj_q:Array[ISet], label_q:Array[Int], adj_g:Array[ISet], label_g:Array[Int])
    :Set [Array [Int]]={
        val q = new Graph (adj_q, label_q)
        val g = new Graph (adj_g, label_g)
        val matcher = new UllmannIso (g,q)
        return matcher.bijections()
    }
}